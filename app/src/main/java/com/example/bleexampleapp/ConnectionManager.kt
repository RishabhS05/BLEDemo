...
object ConnectionManager {
    ...
    fun registerListener(listener : ConnectionEventListener) {
        ...
        DebugLog.d("Added listener $listener, ${listeners.size} listeners total")
    }
    ...
    fun unregisterListener(listener : ConnectionEventListener) {
        ...
        toRemove?.let {
            listeners.remove(it)
            DebugLog.d("Removed listener ${it.get()}, ${listeners.size} listeners total")
        }
    }

    fun connect(device : BluetoothDevice, context : Context) {
        if(device.isConnected()) {
            DebugLog.e("Already connected to ${device.address}!")
        } else {
    ...
    fun teardownConnection(device : BluetoothDevice) {
        if(device.isConnected()) {
            ...
            DebugLog.e("Not connected to ${device.address}, cannot teardown connection!")
        }
    }
    ...
    fun readCharacteristic(device : BluetoothDevice, characteristic : BluetoothGattCharacteristic) {
        if(device.isConnected() && characteristic.isReadable()) {
            ...
            DebugLog.e("Attempting to read ${characteristic.uuid} that isn't readable!")
        } else if(!device.isConnected()) {
            DebugLog.e("Not connected to ${device.address}, cannot perform characteristic read")
        }
    }
    ...
    fun writeCharacteristic(
        ...
        val writeType = when {
            ...
            characteristic.isWritableWithoutResponse() -> {
                ...
                DebugLog.e("Characteristic ${characteristic.uuid} cannot be written to")
                return
            }
        }
        if(device.isConnected()) {
            ...
            DebugLog.e("Not connected to ${device.address}, cannot perform characteristic write")
        }
    }
    ...
    fun readDescriptor(device : BluetoothDevice, descriptor : BluetoothGattDescriptor) {
        if(device.isConnected() && descriptor.isReadable()) {
            ...
            DebugLog.e("Attempting to read ${descriptor.uuid} that isn't readable!")
        } else if(!device.isConnected()) {
            DebugLog.e("Not connected to ${device.address}, cannot perform descriptor read")
        }
    }
    ...
    fun writeDescriptor(
        ...
        if(device.isConnected() && (descriptor.isWritable() || descriptor.isCccd())) {
            ...
            DebugLog.e("Not connected to ${device.address}, cannot perform descriptor write")
        } else if(!descriptor.isWritable() && !descriptor.isCccd()) {
            DebugLog.e("Descriptor ${descriptor.uuid} cannot be written to")
        }
    }
    ...
    fun enableNotifications(device : BluetoothDevice, characteristic : BluetoothGattCharacteristic) {
        if(device.isConnected() &&
            ...
            DebugLog.e("Not connected to ${device.address}, cannot enable notifications")
        } else if(!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            DebugLog.e("Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun disableNotifications(device : BluetoothDevice, characteristic : BluetoothGattCharacteristic) {
        if(device.isConnected() &&
            ...
            DebugLog.e("Not connected to ${device.address}, cannot disable notifications")
        } else if(!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            DebugLog.e("Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun requestMtu(device : BluetoothDevice, mtu : Int) {
        if(device.isConnected()) {
            ...
            DebugLog.e("Not connected to ${device.address}, cannot request MTU update!")
        }
    }
    ...
    private fun signalEndOfOperation() {
        DebugLog.d("End of $pendingOperation")
    ...
    private fun doNextOperation() {
        if(pendingOperation != null) {
            DebugLog.e("doNextOperation() called when an operation is pending! Aborting.")
            return
        }
        val operation = operationQueue.poll() ?: run {
            DebugLog.v("Operation queue empty, returning")
            return
        }
        ...
        if(operation is Connect) {
            with(operation) {
                DebugLog.w("Connecting to ${device.address}")
        ...
        val gatt = deviceGattMap[operation.device]
                ?: this@ConnectionManager.run {
                    DebugLog.e("Not connected to ${operation.device.address}! Aborting $operation operation.")
        ...
        when(operation) {
            is Disconnect -> with(operation) {
                DebugLog.w("Disconnecting from ${device.address}")
                gatt.close()
            ...
            is CharacteristicWrite -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    DebugLog.e("Cannot find $characteristicUuid to write to")
                    signalEndOfOperation()
                }
            }
            is CharacteristicRead -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    DebugLog.e("Cannot find $characteristicUuid to read from")
                    signalEndOfOperation()
                }
            }
            is DescriptorWrite -> with(operation) {
                gatt.findDescriptor(descriptorUuid)?.let { descriptor ->
                    ...
                    DebugLog.e("Cannot find $descriptorUuid to write to")
                    signalEndOfOperation()
                }
            }
            is DescriptorRead -> with(operation) {
                gatt.findDescriptor(descriptorUuid)?.let { descriptor ->
                    ...
                    DebugLog.e("Cannot find $descriptorUuid to read from")
                    signalEndOfOperation()
                }
            }
                ...
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    characteristic.getDescriptor(cccdUuid)?.let { cccDescriptor ->
                        if(!gatt.setCharacteristicNotification(characteristic, true)) {
                            DebugLog.e("setCharacteristicNotification failed for ${characteristic.uuid}")
                            signalEndOfOperation()
                            return
                        }
                        ...
                        DebugLog.e("${characteristic.uuid} doesn't contain the CCC descriptor!")
                        signalEndOfOperation()
                    }
                } ?: this@ConnectionManager.run {
                    DebugLog.e("Cannot find $characteristicUuid! Failed to enable notifications.")
                    signalEndOfOperation()
                }
            }
            is DisableNotifications -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    DebugLog.e("Cannot find $characteristicUuid! Failed to disable notifications.")
                    signalEndOfOperation()
                }
            }
    ...
    private val callback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt : BluetoothGatt, status : Int, newState : Int) {
            ...
            if(status == BluetoothGatt.GATT_SUCCESS) {
                if(newState == BluetoothProfile.STATE_CONNECTED) {
                    DebugLog.w("onConnectionStateChange: connected to $deviceAddress")
                    deviceGattMap[gatt.device] = gatt
                    Handler(Looper.getMainLooper()).post {
                        gatt.discoverServices()
                    }
                } else if(newState == BluetoothProfile.STATE_DISCONNECTED) {
                    DebugLog.e("onConnectionStateChange: disconnected from $deviceAddress")
                    teardownConnection(gatt.device)
                }
            } else {
                DebugLog.e("onConnectionStateChange: status $status encountered for $deviceAddress!")
        ...
        override fun onServicesDiscovered(gatt : BluetoothGatt, status : Int) {
            with(gatt) {
                if(status == BluetoothGatt.GATT_SUCCESS) {
                    DebugLog.w("Discovered ${services.size} services for ${device.address}.")
                    printGattTable()
                    requestMtu(device, GATT_MAX_MTU_SIZE)
                    listeners.forEach { it.get()?.onConnectionSetupComplete?.invoke(this) }
                } else {
                    DebugLog.e("Service discovery failed due to status $status")
                    teardownConnection(gatt.device)
                }
            }
...
//                        val testid : BluetoothGattDescriptor = characteristic.getDescriptor(UUID.fromString(CLIENT_CHARACTERISTIC_CONFIG))
                        DebugLog.i("Read characteristic $uuid | value: ${value.toHexString()}")
//                        gatt.readDescriptor(testid)
                        listeners.forEach { it.get()?.onCharacteristicRead?.invoke(gatt.device, this) }
                    }
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        DebugLog.e("Read not permitted for $uuid!")
                    }
                    else -> {
                        DebugLog.e("Characteristic read failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onCharacteristicWrite(
            ...
            with(characteristic) {
                when(status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        DebugLog.i("Wrote to characteristic $uuid | value: ${value.toHexString()}")
                        listeners.forEach { it.get()?.onCharacteristicWrite?.invoke(gatt.device, this) }
                    }
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        DebugLog.e("Write not permitted for $uuid!")
                    }
                    else -> {
                        DebugLog.e("Characteristic write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onCharacteristicChanged(
            ...
            with(characteristic) {
                DebugLog.i("Characteristic $uuid changed | value: ${value.toHexString()}")
        ...
        override fun onDescriptorRead(
            ...
            with(descriptor) {
                when(status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        DebugLog.i("Read descriptor $uuid | value: ${value.toHexString()}")
                    ...
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        ...
                        DebugLog.e("Descriptor read failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onDescriptorWrite(
            ...
            with(descriptor) {
                when(status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        DebugLog.i("Wrote to descriptor $uuid | value: ${value.toHexString()}")
                    ...
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        ...
                        DebugLog.e("Descriptor write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        private fun onCccdWrite(
            ...
            when {
                notificationsEnabled -> {
                    DebugLog.w("Notifications or indications ENABLED on $charUuid")
                    listeners.forEach {
                ...
                notificationsDisabled -> {
                    DebugLog.w("Notifications or indications DISABLED on $charUuid")
                    listeners.forEach {
                    ...
                    DebugLog.e("Unexpected value ${value.toHexString()} on CCCD of $charUuid")
                }
            }
        }
    }
        ...
        override fun onReceive(context : Context, intent : Intent) {
            with(intent) {
                if(action == BluetoothDevice.ACTION_BOND_STATE_CHANGED) {
                    ...
                    DebugLog.w("${device?.address} bond state changed | $bondTransition")
                }
            }
        }
